...
class membershipHistoryTableViewController: UITableViewController {
    var dataSources=[memberHistory]()
    ...
    func getHistory()
    {
        ...
        let object = RestClient.postRequest(method:HTTPMethod.get,url: "api/user/activity/subscription/history/",header:header, parameters: nil,completion: { [weak self] data in
            ...
            if(data != nil)
            {
                ...
                if(status).boolValue
                {
                    ...
                    {
                        self?.appendHistories(histories: Helper.getMemberHistory(values: result["values"].arrayValue))
    ...
    func appendHistories(histories:[memberHistory])
    {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if(indexPath.row==0)
        {
            ...
            return cell
              ...
              let cell = tableView.dequeueReusableCell(withIdentifier: "rowCell", for: indexPath) as! membershipHistoryRowCell
               self.tableView.rowHeight=40
               let historyItem=dataSources[indexPath.row-1]
              cell.startDate.text=historyItem.subscriptionDate
              cell.endDate.text=historyItem.subscriptionEndDate
               cell.points.text=String(historyItem.pointsUsed!)+" POINTS"
